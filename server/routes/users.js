...
const userManagementRbacDisabled = String(process.env.RBAC_USER_MANAGEMENT_DISABLED || 'false').trim().toLowerCase() === 'true';
...
function normalizePermissionConfig(input) {
  ...
  const sourceTabPermissions = source.tabPermissions && typeof source.tabPermissions === 'object'
    ...
    : {};
  const sourceUserTabPermissions = source.userTabPermissions && typeof source.userTabPermissions === 'object'
    ? source.userTabPermissions
    : {};
  ...
  for (const [userId, value] of Object.entries(sourceUserTabPermissions)) {
    const normalizedUserId = String(userId || '').trim();
    if (!normalizedUserId || normalizedUserId.length > 80) continue;
    ...
    userTabPermissions[normalizedUserId] = normalizedTabs;
  }

  return { roles, tabPermissions, userTabPermissions };
}

function mapAuthRoleToPermissionRole(permissionConfig, authRole) {
  ...
  const role = normalizeRoleId(authRole);
  if (allowedRoleSet.has(role)) return role;
  if (role === 'member' && allowedRoleSet.has('viewer')) return 'viewer';
  if (role === 'approver' && allowedRoleSet.has('operator')) return 'operator';
  if (role === 'admin' && allowedRoleSet.has('admin')) return 'admin';
  return fallbackRole;
}

async function getCurrentRoleFromSetting(permissionConfig) {
  const allowedRoleIds = permissionConfig.roles.map((role) => role.id);
  const allowedRoleSet = new Set(allowedRoleIds);
  const fallbackRole = allowedRoleIds[0] || 'operator';
  ...
  return allowedRoleSet.has(role) ? role : fallbackRole;
}

function computeAllowedTabs(permissionConfig, role, userId) {
  const userIdText = String(userId || '').trim();
  const userTabs = Array.isArray(permissionConfig.userTabPermissions?.[userIdText])
    ? permissionConfig.userTabPermissions[userIdText]
    : null;

  if (userTabs && userTabs.length > 0) {
    return {
      source: 'user',
      allowedTabs: Array.from(new Set(userTabs.map((tabId) => String(tabId || '').trim()).filter(Boolean)))
    };
  }

  const allowedTabs = Object.entries(permissionConfig.tabPermissions || {})
    .filter(([, roleIds]) => Array.isArray(roleIds) && roleIds.includes(role))
    .map(([tabId]) => tabId);

  return {
    source: 'role',
    allowedTabs: Array.from(new Set(allowedTabs))
  };
}
...
// GET /api/users/my-tab-access - 获取当前登录用户可访问菜单
router.get('/my-tab-access', async (req, res) => {
  try {
    const config = await getPermissionConfig();
    const role = mapAuthRoleToPermissionRole(config, req.authUser?.role);
    const userId = String(req.authUser?.id || '').trim();
    const result = computeAllowedTabs(config, role, userId);
    res.json({
      userId,
      role,
    ...
    console.error('获取当前用户菜单访问权限失败:', error);
    res.status(500).json({ error: error.message || '获取菜单权限失败' });
  }
});
