    ...
    <div class="user-grid">
      ...
      <section class="panel user-edit">
        <div v-if="selectedUser">
          ...
          <div class="user-menu-permission">
            <div class="user-menu-head">
              <strong>用户菜单授权（覆盖角色默认权限）</strong>
              <button class="btn" type="button" @click="resetUserTabPermissions">恢复角色默认</button>
            </div>
            <div class="user-menu-grid">
              <label v-for="tabId in tabIds" :key="`user-tab-${tabId}`" class="checkbox-inline">
                <input
                  type="checkbox"
                  :checked="selectedUserTabPermissions.includes(tabId)"
                  @change="toggleUserTabPermission(tabId)"
                />
                <span>{{ tabId }}</span>
              </label>
            </div>
...
export default {
  ...
  computed: {
    ...
    tabIds() {
      return Object.keys(this.permissionConfig.tabPermissions || {})
        .sort((a, b) => a.localeCompare(b, 'zh-CN'))
    },
    selectedUserTabPermissions() {
  ...
  methods: {
    ...
    async loadPermissionConfig() {
      try {
        ...
        this.permissionConfig = {
          ...
          userTabPermissions: config?.userTabPermissions || {}
        }
    ...
    toggleUserTabPermission(tabId) {
      if (!this.selectedUser?.id) return
      const userId = this.selectedUser.id
      const current = new Set(this.selectedUserTabPermissions)
      if (current.has(tabId)) current.delete(tabId)
      else current.add(tabId)
      this.permissionConfig = {
        ...this.permissionConfig,
        userTabPermissions: {
          ...(this.permissionConfig.userTabPermissions || {}),
          [userId]: Array.from(current)
        }
      }
    },
    resetUserTabPermissions() {
      if (!this.selectedUser?.id) return
      const userId = this.selectedUser.id
      const next = { ...(this.permissionConfig.userTabPermissions || {}) }
      delete next[userId]
      this.permissionConfig = {
        ...this.permissionConfig,
        userTabPermissions: next
      }
    },
