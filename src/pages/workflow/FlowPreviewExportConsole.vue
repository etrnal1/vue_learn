<template>
  <div class="flow-preview-export">
    <header class="flow-preview-header">
      <div>
        <p class="eyebrow">工作流 › 流程</p>
        <h1>流程</h1>
        <p class="subtitle">
          浏览流程定义，查看步骤详情，并按需导出 JSON 或概要文档用于评审与归档。
        </p>
      </div>
      <div class="header-actions">
        <button class="btn btn-outline" :disabled="loading" @click="refresh">
          刷新
        </button>
        ...
        <div v-if="selectedFlow" class="preview-card">
          <div class="preview-card__header">
            ...
            <div class="preview-card__actions">
              <button class="btn btn-ghost" @click="openPreviewWindow">打开</button>
      ...
      <aside class="preview-side">
        <div class="inspector">
          <div class="inspector-header">
            ...
            <h3>{{ selectedFlow?.name || '未选择流程' }}</h3>
            <p class="subtitle inspector-subtitle">查看详情、发起导出，并跟踪审计日志。</p>
          </div>
          <div class="inspector-tabs">
            <button
              class="inspector-tab"
              :class="{ active: inspectorTab === 'info' }"
            ...
            <button
              class="inspector-tab"
              :class="{ active: inspectorTab === 'export' }"
              @click="inspectorTab = 'export'"
            >
              导出
            </button>
            <button
              class="inspector-tab"
              :class="{ active: inspectorTab === 'audit' }"
              @click="inspectorTab = 'audit'"
            >
              审计日志
            </button>
          </div>
          ...
          <div class="inspector-body">
            <section v-show="inspectorTab === 'info'" class="meta-panel">
              <dl>
                <div>
                  <dt>状态</dt>
            ...
            <section v-show="inspectorTab === 'export'" class="meta-panel">
            ...
            <section v-show="inspectorTab === 'audit'" class="meta-panel meta-panel__log">
              <div class="audit-header">
                <h4>审计日志</h4>
                <button class="btn btn-small btn-secondary" @click="refreshAuditLog">刷新</button>
              </div>
              ...
              <ul v-else>
                <li v-for="entry in activityLog" :key="entry" class="audit-entry audit-entry--fallback">
                  <span class="log-action">{{ entry }}</span>
                </li>
              </ul>
            </section>
          </div>
...
export default {
  ...
  methods: {
    ...
    selectFlow(flow) {
      ...
      this.inspectorTab = this.selectedFlow ? this.inspectorTab : 'info'
    },
    refreshAuditLog() {
    ...
    statusLabel(flow) {
      const status = this.getFlowStatus(flow)
      if (status === 'published' || status === 'active') return '启用'
